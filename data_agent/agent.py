...
def fig_inter(py_code: str, fname: str) -> str:
    ...
    ⚠️ 重要使用規則：
    1. **變數命名規則**：繪圖程式碼必須將最終的圖像物件 (Figure) 賦值給變數 `fig`。
       (例如：必須使用 `fig = plt.figure()` 或 `fig, ax = plt.subplots()`)
    2. **嚴禁顯示**：絕對不要使用 `plt.show()`，否則會導致系統崩潰。
    3. **佈局**：程式碼末尾建議呼叫 `fig.tight_layout()` 確保佈局緊湊。
    4. **語言**：圖表中的所有文本 (標題、標籤) 建議使用英文。
    ...
    :param py_code: 要執行的 Python 繪圖程式碼 (必須將圖形物件存為變數 `fig`)。
    :param fname: 圖片存檔的名稱 (不需副檔名)，例如 'sales_trend'。
    :return: 圖片儲存結果的路徑說明。
    """
    ...
    try:
        ...
        # 如果使用者用 seaborn 的 JointGrid 或 PairGrid，變數名通常也建議叫 fig 或 g
        # 這裡做一個容錯：如果找不到 fig，嘗試找 'g' (seaborn 常見慣例)，或者檢查是否只有一個 Figure 物件
        if fig is None:
            # 嘗試尋找 environment 中是否只有一個 Figure 物件，如果有就直接用它
            figures = [
                v
                for v in local_vars.values()
                if isinstance(v, (plt.Figure, sns.JointGrid, sns.PairGrid))
            ]
            if len(figures) == 1:
                fig = figures[0]
        ...
        if fig and hasattr(fig, "savefig"):
            # 檔名清理 (防止路徑攻擊或無效字元)
            safe_fname = "".join(
                [c for c in fname if c.isalnum() or c in ("_", "-")]
            ).strip()
            if not safe_fname:
                safe_fname = "output_plot"

            image_filename = f"{safe_fname}.png"
            ...
            return f"⚠️ 程式碼執行完畢，但未找到名為 'fig' 的圖像物件。請確保程式碼包含 `fig = ...`。"
...
* **展示結果**:
    * 如果成功生成了圖片 (來自 `fig_inter`)，你**必須**在回答中使用 Markdown 格式插入圖片。
        * 範例： `![用戶流失的類別特徵分析](這邊是圖片的相對路徑，並且前面要以 http://localhost:2024/ 開頭，舉例：http://localhost:2024/images/example.png)`
    * **絕對不要**只輸出圖片的路徑文字。
* **風格**: 保持專業、簡潔、以數據為導向。
"""
